...
function comfyProxyServiceWorkerScript() {
  return `const proxyPrefix = ${scriptSafeJson(COMFY_PROXY_PREFIX)};
...
(() => {
  ...
  if ('serviceWorker' in navigator) {
    ...
    const infinitySwPath = ${scriptSafeJson(COMFY_PROXY_PREFIX)} + '__infinity_sw.js';
    const infinitySwUrl =
      infinitySwPath + (infinitySwToken ? '?' + proxyTokenParam + '=' + encodeURIComponent(infinitySwToken) : '');
    navigator.serviceWorker.register(infinitySwUrl, { scope: ${scriptSafeJson(COMFY_PROXY_PREFIX)} }).catch(() => {});
...
async function handleComfyProxy(request, response) {
  try {
    const swPath = `${COMFY_PROXY_PREFIX}__infinity_sw.js`
    if (new URL(request.url ?? '/', 'http://infinity.local').pathname === swPath) {
