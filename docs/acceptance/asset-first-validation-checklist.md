...
| 2026-06-24 | `npm test -- src/components/canvas/CanvasWorkspace.test.tsx -t "opens a compatible function menu"` | FAIL -> PASS | Reproduced and fixed asset context-menu function actions losing the right-clicked asset when React Flow emits an empty selection change. |
| 2026-06-24 | `npm run browser:smoke -- --reporter=line` | PASS | Browser smoke now verifies right-clicking an image asset filters to compatible functions and opens the Image Info command modal with the asset preselected. |
