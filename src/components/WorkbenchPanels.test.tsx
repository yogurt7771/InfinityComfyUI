...
describe('LeftPanel', () => {
  ...
  it('keeps workflow creation blocked while the embedded ComfyUI editor has not exported a workflow', () => {
    ...
    expect(decodeURIComponent(frameUrl.pathname)).toContain('http://127.0.0.1:8188')
    expect(frameUrl.searchParams.get('__infinity_comfy_token')).toBe('remote-editor-token')
  ...
  it('keeps a selected workflow unchanged while its embedded ComfyUI editor is open', () => {
    ...
    expect(frameUrl.pathname).toContain('/__comfy_proxy/')
    expect(decodeURIComponent(frameUrl.pathname)).toContain('http://127.0.0.1:8188')
    expect(frameUrl.searchParams.get('__infinity_comfy_token')).toBe('selected-workflow-token')
    expect(openSpy).not.toHaveBeenCalled()
    expect(submitSpy).not.toHaveBeenCalled()
    expect(projectStore.getState().project.functions.fn_render.workflow).toEqual(originalWorkflow)
    expect(fetchMock).not.toHaveBeenCalled()
  })
  it('loads the workflow after the ComfyUI app-ready message using a frame-realm File', async () => {
    ...
    await waitFor(
      () => expect(within(editorDialog).getByRole('button', { name: 'Save from ComfyUI' })).toBeEnabled(),
      { timeout: 5000 },
    )
