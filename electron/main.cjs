...
const LOCAL_APP_PORT = Math.max(1, Number(process.env.INFINITY_LOCAL_PORT) || 7930)
const LOCAL_APP_HEALTH_PATH = '/__infinity_health'
const browserLauncherMode = process.env.INFINITY_APP_MODE === 'launcher'
const hasSingleInstanceLock = !browserLauncherMode || app.requestSingleInstanceLock()
let localAppServer
...
const startAppServer = () =>
  new Promise((resolve, reject) => {
    ...
    localAppServer = http.createServer((request, response) => {
      void (async () => {
        try {
          ...
          if (requestUrl.pathname === LOCAL_APP_HEALTH_PATH) {
            response.setHeader('content-type', 'application/json; charset=utf-8')
            response.end(JSON.stringify({ application: 'Infinity ComfyUI', proxy: 'v5.17.1' }))
            return
          }
    ...
    localAppServer.on('error', handleServerError)
    localAppServer.on('listening', () => {
    ...
    localAppServer.listen(LOCAL_APP_PORT, '127.0.0.1')
  })
...
const loadProjectLibrary = async () => {
  ...
  if (
    library !== undefined &&
    (!library || typeof library !== 'object' || !library.projects || typeof library.projects !== 'object')
  ) {
...
const createLauncher = async () => {
  const appUrl = rendererDevelopmentUrl() ?? (await startAppServer())
  launcherTray = new Tray(appIconPath())
  launcherTray.setToolTip('Infinity ComfyUI Launcher')
  launcherTray.setContextMenu(
    Menu.buildFromTemplate([
      {
        label: 'Open Infinity ComfyUI',
  ...
  launcherTray.on('double-click', () => void shell.openExternal(appUrl))
  await shell.openExternal(appUrl)
}
...
if (!hasSingleInstanceLock) {
  ...
  app.on('second-instance', () => {
    if (browserLauncherMode) void openLauncherPage()
  })

  app.whenReady().then(() => {
    const launch = browserLauncherMode ? createLauncher() : createWindow()
    ...
    app.on('activate', () => {
      if (browserLauncherMode) {
        void openLauncherPage()
...
app.on('window-all-closed', () => {
  if (!browserLauncherMode && process.platform !== 'darwin') app.quit()
})

app.on('before-quit', () => {
  launcherTray?.destroy()
  launcherTray = undefined
