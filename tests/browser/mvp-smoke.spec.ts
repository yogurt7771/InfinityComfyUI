...
test('supports asset-first canvas creation, preview, drop, and replacement', async ({ page }) => {
  ...
  const singleDragAssetCount = await page.locator('.react-flow__node-asset').count()
  await expect.poll(async () => visibleAssetNodeCount(page)).toBe(singleDragAssetCount)
  await expect(page.locator('.comfy-minimap-node-asset')).toHaveCount(singleDragAssetCount)
  await page.mouse.up()
  ...
  await expect(page.getByRole('menuitem', { name: 'Image Info' })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: 'Text Case' })).toHaveCount(0)
  await page.getByRole('menuitem', { name: 'Image Info' }).click()
  const functionCommandDialog = page.getByRole('dialog', { name: 'Image Info function command' })
  await expect(functionCommandDialog).toBeVisible()
  await expect(functionCommandDialog.getByLabel('Selected assets')).toContainText('Image')
  await page.getByRole('button', { name: 'Close function command' }).click()
  await expect(functionCommandDialog).toHaveCount(0)
  ...
  await page.mouse.down()
  await page.mouse.move(1960, 930, { steps: 24 })
  await expect.poll(async () => visibleAssetNodeCount(page)).toBe(afterBatchCount)
